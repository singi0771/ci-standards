...
| 限制 | 影響 | 現在該怎麼辦 |
|---|---|---|
| [Copilot Coding Agent 不回應 Actions 貼的 `@copilot`](#copilot-coding-agent-對-actions-貼的-copilot-沒有反應) | 「自動修」半條迴圈 | 當成人工修，先導入其餘部分 |
...
- `@copilot` mention 後等 **12 分鐘**，Coding Agent 沒有任何動作（沒有 commit、沒有 agent session）
- 改用 GraphQL `replaceActorsForAssignable` 直接把 Copilot 指派到該 PR，等 **9 分鐘**，同樣沒反應

### ⚠️ 上面第二個測試其實無效，別拿它當結論
...
所以「直接指派也沒反應」證明不了「bot mention 被擋」，只證明了「那個入口不存在」。
**排查必須從頭來過。**

### 排查步驟（一定要照順序，前一關沒過後面不用試）

**第 1 關 —— Coding Agent 到底有沒有開？**
...
**第 2 關 —— Agent 可用，但不吃 bot 的留言**

第 1 關過了（指派 Issue 會動），但 Actions 貼的 `@copilot` 還是沒反應 —— 這才是 GitHub 的
bot 防迴圈限制。這時才換 token：
...
那就接受現實：Coding Agent 只從 Issue 進入。把流程改成
「掃描發現弱點 → 人工（或另一支 workflow）開 Issue → 指派 Copilot」，
把 `copilot-autofix-*` 從 consumer 範本移除或維持不啟用。

### 在這件事解決之前，怎麼導入

**照常導入，只是不要期待「自動修」。** 這條線路現在是可用的：

```
免費掃描器「找」→ Security Gate 擋門 → Copilot Code Review「審」→ 人「修」→ 人 merge
```
